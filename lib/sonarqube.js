...
function checkQualityGateStatus(params) {
  ...
  const srvUrl = new URL('api/qualitygates/project_status', params['sonar.host.url']);
  srvUrl.searchParams.set('projectKey', params['sonar.projectKey']);

  if (params['sonar.pullRequest']) {
    srvUrl.searchParams.append('pullRequest', params['sonar.pullRequest']);
  } else if (params['sonar.branch']) {
    srvUrl.searchParams.append('branch', params['sonar.branch']);
  }

  return checkReportStatus(login, password).then(reportStatus => {
    debug('reportStatus : ' + reportStatus);
    console.log('Check the Quality gate ' + srvUrl.href);
    ...
    return new Promise((resolve, reject) => {
      const req = requestModule.request(srvUrl.href, options, response => {
